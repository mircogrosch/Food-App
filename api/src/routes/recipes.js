...
router.get("/", async (req, res) => {
  ...
  if (name) {
    try {
      ...
      const responses = [...responseDbData, responseApiData].flat();
  ...
  if (idReceta) {
    ...
    try {
      try {
        //llamada a la api externa
        let responseApi = await axios.get(
          `${URL}/${idReceta}/information?apiKey=${process.env.API_KEY}`
        );
        ...
        const apiData = {
          ...
          steps:
            responseApi.data.analyzedInstructions.length > 0
              ? responseApi.data.analyzedInstructions[0].steps.map((e) => {
    ...
    dbData.forEach((el) => {
      ...
      responseDbData=data;
        ...
        return res.json(responseDbData);
      }
    } catch (err) {
      res.send(err);
    }
